...
parse_sbat_var(list_t *entries, char *sbat_var_candidate)
{
	...
	if (sbat_var_candidate == NULL) {
		...
		efi_status = get_variable(SBAT_VAR_NAME, &data, &datasize,
		                          SHIM_LOCK_GUID);
		if (EFI_ERROR(efi_status)) {
	...
	/*
	 * We've intentionally made sure there's a NULL byte on all variable
...
bool
preserve_ssp_uefi_variable(UINT8 *ssp_applied, UINTN sspversize,
                           UINT32 attributes, uint8_t *ssp_candidate)
{
	UINT64 old, new;
...
set_sbat_uefi_variable(char *sbat_var_previous, char *sbat_var_latest)
{
	...
	if (EFI_ERROR(efi_status)) {
		...
		if (EFI_ERROR(efi_status)) {
			CHAR16 *title = L"New SbatLevel would self-revoke "
					"current shim. Not applied";
	...
	/* set variable */
	efi_status =
		set_variable(SBAT_VAR_NAME, SHIM_LOCK_GUID, SBAT_VAR_ATTRS,
                      ...
                      uint8_t *ssp_ver_latest, uint8_t *ssp_sig_latest)
{
	...
	if (ssp_policy == POLICY_NOTREAD) {
		efi_status =
			get_variable_attr(SSP_POLICY, &policyp, &policysize,
		                          SHIM_LOCK_GUID, &attributes);
		if (!EFI_ERROR(efi_status)) {
			ssp_policy = *policyp;
			clear_ssp_policy();
	...
	efi_status =
		get_variable_attr(SSPVER_VAR_NAME, &sspver, &sspversize,
	...
	if (EFI_ERROR(efi_status)) {
	...
	} else if (preserve_ssp_uefi_variable(sspver, sspversize, attributes,
	                                      ssp_ver) &&
	           !reset_ssp) {
		FreePool(sspver);
		...
		dprint("%s variable is %d bytes, attributes are 0x%08x\n",
		       SSPVER_VAR_NAME, sspversize, attributes);
		dprint("Deleting %s variable.\n", SSPVER_VAR_NAME);
		efi_status = set_variable(SSPVER_VAR_NAME,
		                          SECUREBOOT_EFI_NAMESPACE_GUID,
		                          attributes, 0, "");
		if (EFI_ERROR(efi_status)) {
			dprint(L"%s variable delete failed %r\n", SSPVER_VAR_NAME,
					efi_status);
		...
		dprint("Deleting %s variable.\n", SSPSIG_VAR_NAME);
		efi_status = set_variable(SSPSIG_VAR_NAME,
		                          SECUREBOOT_EFI_NAMESPACE_GUID,
		                          attributes, 0, "");
		if (EFI_ERROR(efi_status)) {
			dprint(L"%s variable delete failed %r\n", SSPSIG_VAR_NAME,
	...
	/* set variable */
	efi_status =
		set_variable(SSPVER_VAR_NAME, SECUREBOOT_EFI_NAMESPACE_GUID,
	...
	efi_status =
		set_variable(SSPSIG_VAR_NAME, SECUREBOOT_EFI_NAMESPACE_GUID,
